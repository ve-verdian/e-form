...
class Admin extends CI_Controller {
	...
	public function bagian() {
		...
		$a['pagi']	= _page($total_row, $per_page, 4, base_url()."admin/bagian/p");
	...
	public function insert_user() {
		...
		$a['pagi']	= _page($total_row, $per_page, 4, base_url()."admin/insert_user/p");
		...
		//upload config 
		$config['upload_path'] 		= './upload/insert_user';
		...
		if ($mau_ke == "del") {
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("INSERT INTO t_surat_masuk VALUES (NULL, '$kode', '$no_agenda', '$indek_berkas', '$uraian', '$dari', '$no_surat', '$tgl_surat', NOW(), '$ket', '".$up_data['file_name']."', '".$this->session->userdata('admin_id')."', '$nomor_hp', '$akses_menu')");
			} else {
			...
			if ($this->upload->do_upload('file_surat')) {
				...
				$this->db->query("UPDATE t_surat_masuk SET kode = '$kode', no_agenda = '$no_agenda', indek_berkas = '$indek_berkas', isi_ringkas = '$uraian', dari = '$dari', no_surat = '$no_surat', tgl_surat = '$tgl_surat', keterangan = '$ket', nomor_hp = '$nomor_hp', akses_menu = '$akses_menu' file = '".$up_data['file_name']."' WHERE id = '$idp'");
			} else {
	...
	public function pengguna() {
		...
		if ($mau_ke == "act_edt") {
			if ($this->upload->do_upload('logo')) {
				...
				$this->db->query("UPDATE tr_instansi SET nama = '$nama', alamat = '$alamat', kepsek = '$kepsek', nip_kepsek = '$nip_kepsek', logo = '".$up_data['file_name']."',  telp = '$telp',  fax = '$fax' WHERE id = '$idp'");

			} else {
				$this->db->query("UPDATE tr_instansi SET nama = '$nama', alamat = '$alamat', kepsek = '$kepsek', nip_kepsek = '$nip_kepsek',  telp = '$telp',  fax = '$fax' WHERE id = '$idp'");
			}		
	...
	public function cetak_user_baru() {
	...
	public function do_login() {
        ...
        if($j_cek == 1) {
			...
			$this->session->set_flashdata("k", "<div id=\"alert\" class=\"alert alert-error\">username dan password tidak valid</div>");
			redirect('admin/login');
		}
	}
